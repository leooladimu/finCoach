...
function analyzeSpendingVsGoals(transactions, goals, profile) {
    ...
    const savingsGoals = goals.filter(g => g.category === 'savings' || g.title.toLowerCase().includes('save') || g.title.toLowerCase().includes('fund'));
    if (savingsGoals.length > 0 && discretionaryPercent > 25) {
        ...
        if (monthlyDiscretionary > monthlyTarget * 0.5) {
            contradictions.push({
                ...
                suggestion: adaptSuggestionToPersonality(`Consider reducing discretionary spending by $${(monthlyDiscretionary * 0.3).toFixed(0)}/month. This would accelerate your ${goal.title} by ${calculateMonthsSaved(monthlyDiscretionary * 0.3, goal)} months.`, profile.type),
                emoji: '⚠️',
...
function analyzeCategoryPatterns(transactions, timeRange) {
    ...
    Object.entries(categoryBudgets).forEach(([category, budgetPercent]) => {
        ...
        if (overagePercent > 30) { // 30% over budget
            const severity = overagePercent > 50 ? 'high' : overagePercent > 40 ? 'medium' : 'low';
            contradictions.push({
