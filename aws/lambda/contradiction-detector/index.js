...
const handler = async (event) => {
  console.log("Contradiction Detection Lambda triggered", {
    userId: event.userId,
  });
  try {
    ...
    const spendingContradictions = analyzeSpendingVsGoals(
      transactions,
      goals,
      profile,
    );
    ...
    const categoryContradictions = analyzeCategoryPatterns(
      transactions,
      timeRange,
    );
    contradictions.push(...categoryContradictions);
    // 3. Personality vs Behavior Analysis
    const personalityContradictions = analyzePersonalityAlignment(
      profile,
      transactions,
    );
    ...
    console.log(
      `Detected ${sortedContradictions.length} contradictions for user ${event.userId}`,
    );
    return {
      statusCode: 200,
      headers: {
      ...
      body: JSON.stringify({
        ...
        breakdown: {
          high: sortedContradictions.filter((c) => c.severity === "high")
            .length,
          medium: sortedContradictions.filter((c) => c.severity === "medium")
            .length,
 ...
 */
function analyzeSpendingVsGoals(transactions, goals, profile) {
  const contradictions = [];
  ...
  if (savingsGoals.length > 0 && discretionaryPercent > 25) {
    ...
    if (monthlyDiscretionary > monthlyTarget * 0.5) {
      contradictions.push({
        ...
        emoji: "⚠️",
        hypothesis:
          "User may benefit from automatic savings transfers before discretionary spending",
        resolved: false,
      });
    }
  }
 ...
 */
function analyzeCategoryPatterns(transactions, timeRange) {
  const contradictions = [];
  ...
  Object.entries(categoryBudgets).forEach(([category, budgetPercent]) => {
    ...
    const overagePercent =
      ((spentPercent - budgetPercent) / budgetPercent) * 100;
 ...
 */
function analyzePersonalityAlignment(profile, transactions) {
  const contradictions = [];
  const { type } = profile;
  // Check if Judger (J) but many spontaneous purchases
  if (type.includes("J")) {
    const smallFrequentPurchases = transactions.filter(
      (t) => t.amount < 20,
    ).length;
    const totalTransactions = transactions.length;
    const impulseBuyPercent =
      (smallFrequentPurchases / totalTransactions) * 100;
      ...
      contradictions.push({
        ...
        actual: {
          ...
          evidence: [
            ...
            "Pattern suggests unplanned spending",
          ],
          value: smallFrequentPurchases,
        },
        suggestion:
          'Try batching purchases weekly or using a "planned discretionary" budget to maintain structure while allowing flexibility.',
        emoji: "🎯",
        resolved: false,
      });
    }
 ...
 */
function analyzeGoalProgress(goals, transactions) {
  const contradictions = [];
  goals.forEach((goal) => {
        ...
        contradictions.push({
          ...
          actual: {
            ...
            evidence: [
              ...
              `${monthsUntilTarget.toFixed(0)} months left`,
            ],
 ...
 */
function detectPositivePatterns(transactions, goals, profile) {
  const contradictions = [];
  ...
  goals.forEach((goal) => {
    ...
    if (progressPercent > 75) {
      contradictions.push({
        ...
        suggestion:
          "Keep up the great work! Consider celebrating this milestone in a budget-friendly way.",
        emoji: "🎉",
        resolved: false,
      });
    }
  ...
  if (essentialPercent > 60 && essentialPercent < 75) {
    contradictions.push({
      ...
      suggestion:
        "Your spending balance is healthy! This foundation gives you flexibility to pursue your financial goals.",
      emoji: "✨",
      resolved: false,
    });
  }
 ...
 */
function adaptSuggestionToPersonality(baseSuggestion, mbtiType) {
 ...
 */
function calculateMonthsSaved(monthlyReduction, goal) {
  const remaining = goal.targetAmount - goal.currentAmount;
  const currentMonthsNeeded = remaining / (goal.currentAmount / 12); // rough estimate
  const newMonthsNeeded =
    remaining / (monthlyReduction + goal.currentAmount / 12);
